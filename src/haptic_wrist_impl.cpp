...
HapticWristImpl::HapticWristImpl()
    ...
    controllers_ = {
        ...
        // TODO: will need to read this from ps5 thumbstick somehow, probably need to seperate this out somewhere
    };
...
bool HapticWristImpl::entryPoint() {
    while (running_.load()) {
        ...
        jp_type total_joint_torques = jt_type::Zero();
        ...
        if (current_mode == ControlMode::POSITION) {
            ...
            // TODO: use thumbstick to augment curr pos of last joint?
            ...
            // TODO: use thumbstick to augment curr pos of last joint?
            Eigen::Matrix<double, 3, 2> J_omega = kinematics_.jacobian_omega(current_pos);
            ...
            Eigen::Matrix2d M_inv = M.inverse(); // For a diagonal matrix, this is just 1/m_ii
            ...
            Eigen::Matrix3d Lambda = JM_invJT.inverse();
...
// TODO: this also needs to be changed to pickup ps5 thumbstick
bool HapticWristImpl::executeControl(const mt_type& des_motor_torque) {
    ...
    {
        // TODO: this is probably where we want to stick in ps5 to calculate current state
